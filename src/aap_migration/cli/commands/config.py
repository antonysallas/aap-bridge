...
def _test_vault_connectivity(ctx: MigrationContext) -> None:
    """Test Vault auth and verify referenced secrets exist with required keys."""
    config = ctx.config

    # Only test if Vault is configured
    if config.vault is None:
        echo_info("Vault: not configured, skipping")
        return

    # Authenticate
    echo_info(f"Testing Vault connection: {config.vault.url}")
    try:
        vault = ctx.vault_client  # triggers AppRole auth
        if vault.is_authenticated():
            echo_success("Vault: authenticated via AppRole")
        else:
            raise click.ClickException("Vault authentication failed")

        # Check referenced secrets
        for label, instance in [("source", config.source), ("target", config.target)]:
            if instance.token_vault_path:
                echo_info(f"Checking Vault secret for {label}: {instance.token_vault_path}")
                if vault.validate_credential(instance.token_vault_path, ["token"]):
                    echo_success(f"Vault: {label} secret exists with 'token' key")
                else:
                    raise click.ClickException(
                        f"Vault secret for {label} at '{instance.token_vault_path}' "
                        f"is missing or does not contain a 'token' key"
                    )
    except Exception as e:
        echo_error(f"Vault connectivity test failed: {e}")
        raise click.ClickException(f"Vault connection failed: {e}") from e


def _test_connectivity(ctx: MigrationContext) -> None:
    """Test connectivity to source and target AAP instances."""

    # Test Vault first if needed
    if ctx.config.vault:
        _test_vault_connectivity(ctx)
...
def show(ctx: MigrationContext) -> None:
    ...
    if config.source.token_vault_path:
        click.echo(f"  Token (Vault): {config.source.token_vault_path}")
    else:
        click.echo(f"  Token (Plain): {'*' * 40} (masked)")
    click.echo(f"  Verify SSL: {config.source.verify_ssl}")

    click.echo("\nTarget Configuration:")
    click.echo(f"  URL: {config.target.url}")
    if config.target.token_vault_path:
        click.echo(f"  Token (Vault): {config.target.token_vault_path}")
    else:
        click.echo(f"  Token (Plain): {'*' * 40} (masked)")
    ...
    if config.vault:
        click.echo("\nVault Configuration:")
        click.echo(f"  URL: {config.vault.url}")
        click.echo(f"  Role ID: {config.vault.role_id[:8]}... (masked)")
        click.echo(f"  Mount Point: {config.vault.mount_point}")
        click.echo(f"  Path Prefix: {config.vault.path_prefix}")
        click.echo(f"  Verify SSL: {config.vault.verify_ssl}")
